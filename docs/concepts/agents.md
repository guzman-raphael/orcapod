...
- They support **parallel execution**, allowing different Pods or branches of a
  pipeline to run at the same time.
- Agents send **status signals** back to the orchestrator, including: *progress*, *complete*, *fail*, and *cancel*.
- Execution is designed to be **fault-permissible**: if one Pod fails, only its
  dependent steps are cancelled, while other branches continue.
- The long-term architecture goal is a **mesh of agents**, so there is no single
  point of failure.
